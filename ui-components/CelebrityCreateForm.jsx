...
import { createCelebrity } from "./graphql/mutations";
const client = generateClient();
export default function CelebrityCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setUnionName(initialValues.unionName);
    setFirstPerson(initialValues.firstPerson);
    setSecondPerson(initialValues.secondPerson);
    setLengthInDays(initialValues.lengthInDays);
    setPassingMessage(initialValues.passingMessage);
    setErrors({});
  };
  const validations = {
    unionName: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createCelebrity.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "CelebrityCreateForm")}
      {...rest}
    >
      <TextField
        label="Union name"
